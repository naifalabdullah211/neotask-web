...
void main() {
  group('computeTaskStats — single source of truth invariant', () {
    test(
      'breakdownSum always equals total for a mixed synthetic task set '
      ...
      () {
        ...
        final tasks = <AppTask>[
          ...
          _task(id: '7', status: TaskStatus.approved, dueDate: past), // NOT overdue (completed)
          _task(id: '8', status: TaskStatus.approved, dueDate: future),
          _task(id: '9', status: TaskStatus.rejected, dueDate: future),
          _task(id: '10', status: TaskStatus.rejected, dueDate: past), // overdue AND rejected
          _task(id: '11', status: TaskStatus.editRequested, dueDate: future),
          _task(id: '12', status: TaskStatus.editRequested, dueDate: past), // overdue
        ];
    ...
    test('empty task list produces all-zero stats with breakdownSum==total', () {
    ...
    test(
      'invariant holds across the DAY filter (mirrors '
    ...
    test(
      'invariant holds across the WEEK filter (mirrors '
    ...
    test(
      'a rejected AND overdue task is counted exactly once in breakdownSum '
