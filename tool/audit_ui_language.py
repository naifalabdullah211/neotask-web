...
for root in TARGETS:
    ...
    for path in sorted(root.rglob('*.dart')):
        ...
        risky_props = [(m.group(1), m.group(3).strip()) for m in NON_TEXT_PROPS.finditer(text) if AR.search(m.group(3)) and 'context.tr(' not in m.group(0)]
        is_screen = '/screens/' in f'/{path.as_posix()}'
        legacy = is_screen and 'Scaffold(' in text and 'NeoWorkspace' not in text and 'neo_workspace_chrome.dart' not in text
        ...
        if score:
            examples = '; '.join(f'{k}: {v[:48]}' for k, v in risky_props[:3])
            rows.append((score, path.as_posix(), ', '.join(issues), examples))
